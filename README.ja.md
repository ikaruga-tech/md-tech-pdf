...
CI/CDなどのパイプライン向けスタンドアロンCLIおよびCoreライブラリに加え、VS Code上でリアルタイムに用紙枠プレビューを確認し直接PDF出力できる公式 [VS Code拡張機能](./vscode-extension) を提供しています。

---

## 主な特徴

- **VS Code 拡張機能 & リアルタイム・プレビュー**: 実際の印刷用紙枠（A4、余白）を忠実に再現。エディタ・プレビュー間の双方向スクロール同期（Scroll Sync）、スティッキーツールバー、編集時のスクロール位置保持、高速インメモリキャッシュによる快適なリアルタイムプレビューを提供。
...
v0.4.0 リリース時点で把握されている仕様および制約事項です。
