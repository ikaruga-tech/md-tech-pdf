...
※ 注: 以下の項目のうち、「Scroll Sync」および「プレビュー独自ツールバー（Sticky Toolbar）」は v0.4.0 にて正式に実装されました。

- Scroll Sync（エディタとプレビューのスクロール位置自動同期）：v0.3.0初期リリースでは必須とせず、将来フェーズでの検討対象とする（※ v0.4.0 で実装完了）。
- Custom Editor：Markdownエディタ自体を置き換える実装は行わない。
- プレビュー内でのWYSIWYG編集やダイアグラム直接編集UI。
- プレビュー専用の重厚な独自ツールバーUI（※ v0.4.0 にて軽量なスティッキーツールバーとして実装完了）。
